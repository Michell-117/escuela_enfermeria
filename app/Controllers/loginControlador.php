...
class loginControlador {
    ...
    // =======================================================================
    // =======================================================================
    public function validacion_inicio($data){
        $usuario = $data["usuario_login"];
        $password = strval($data["password_login"]);



        try{
            $consulta_grupo = $this->conecction->prepare("SELECT cuenta, contrasena, tipo FROM login_usuarios WHERE(cuenta = :cuenta && contrasena = :contrasena)");
            $consulta_grupo->bindValue(":cuenta",$usuario);
            $consulta_grupo->bindValue(":contrasena",$password);

            $consulta_grupo->execute();
            $usuario = $consulta_grupo->fetchAll();

            // var_dump($usuario);

            $_SESSION["usuario_cuenta"] = $usuario[0]["cuenta"] ?? "";
            $_SESSION["usuario_tipo"] = $usuario[0]["tipo"] ?? "";

        }catch(Exception $e){
            
        }

        $cuenta = $_SESSION["usuario_cuenta"];
        ...
        if($cuenta == "" && $tipo == ""){
            require "../resources/views/inicio/errorUsuario.php";
        }else{
            require "../resources/views/inicio/usuario.php"; 
        }

        // $user = "RUAM930729";
        // $password = "334455112277";

        // $usuario = $data["usuario_login"];
        // $password1 = $data["password_login"];

        // if ($user == $usuario && $password == $password1 ) {
        //     require "../resources/views/inicio/usuario.php";       
        //  }else{
        //     require "../resources/views/inicio/errorUsuario.php";
        //  }
    ...
    // =======================================================================
    // =======================================================================
    public function inicio_exito($data){
        $usuario = $data["usuario_login"];
        ...
        // =======================================================================
    // =======================================================================
    public function inicio_fallido(){
        require "../resources/views/inicio/errorUsuario.php";
    }
